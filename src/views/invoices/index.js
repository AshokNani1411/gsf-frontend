...
const InvoiceComponent = () => {
  ...
  useEffect(() => {
    if (userData) {
      dispatch(retriveInvoices(userData.X3SITE_0, userData.X3USER_0))
    }
  }, [])
  ...
  const handleFilter = e => {
    ...
    if (value.length) {
      updatedData = invoices && invoices.filter(invoice => {
        let isMatch = false
  ...
  const columns = [
    ...
    },
    {
      ...
      minWidth: '150px',
      cell: row => {
                         return (row.AMTATIL_0 && row.AMTATIL_0 !== " ") ? `${row.AMTATIL_0} ${row.CUR_0}` : ''
                        }
    },
  ...
  const CustomPagination = () => (
    <ReactPaginate
      ...
      pageCount={(searchValue.length || (fromDate && toDate) || selectedStatus || dueDate) ? filteredData.length / 10 : invoices.length / 10 || 1}
      breakLabel={'...'}
  ...
  const onDateChange = (date, type) => {
    if (type === 'from') {
      ...
      if (toDate) {
        ...
        updatedData = invoices && invoices.filter(item => moment(moment(item.ACCDAT_0).format('YYYY-MM-DD')).isBetween(startDate, endDate, undefined, '[]'))
        setFilteredData(updatedData)
      } else {
            ...
            let updatedData = []
                             ...
                             updatedData = invoices && invoices.filter(item => moment(moment(item.ACCDAT_0).format('YYYY-MM-DD')).isSameOrAfter(startDate))
      ...
      if (fromDate) {
        ...
        setFilteredData(updatedData)
            ...
            let updatedData = []
                           const endDate = moment(date[0]).format('YYYY-MM-DD')
                           updatedData = invoices && invoices.filter(item => moment(moment(item.ACCDAT_0).format('YYYY-MM-DD')).isSameOrBefore(endDate))
                                   setFilteredData(updatedData)
  ...
  const onDueDateChange = date => {
    setDueDate(date)
    let updatedData = []
    updatedData = invoices && invoices.filter(item => moment(moment(item.STRDUDDAT_0).format('YYYY-MM-DD')).isSame(moment(date[0]).format('YYYY-MM-DD')))
    setFilteredData(updatedData)
  }
  ...
  const onStatusChange = event => {
    if (event && event.value) {
      ...
      let updatedData = []
      updatedData = invoices && invoices.filter(item => Number(item.Status) === Number(event.value))
      setFilteredData(updatedData)
    } else {
      setStatus('')
        ...
        <Col sm='12'>
          <Card>
            ...
            <DataTable
              noHeader
              pagination
              data={(searchValue.length || fromDate || toDate || selectedStatus || dueDate) ? filteredData : invoices}
              expandableRows
              columns={columns}
