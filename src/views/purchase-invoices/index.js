...
import { retrivePurchaseInvoices } from "../../redux/actions/purchase-invoice/index"
...
const InvoiceComponent = () => {
  ...
  useEffect(() => {
    if (userData) {
      dispatch(retrivePurchaseInvoices(userData.X3SITE_0, userData.X3USER_0))
    }
  }, [])
  ...
  const handleFilter = e => {
    ...
    if (value.length) {
      updatedData = purchaseInvoices && purchaseInvoices.filter(purchaseInvoice => {
        let isMatch = false
        Object.keys(purchaseInvoice).forEach(key => {
          if (purchaseInvoice[key] && purchaseInvoice[key] !== " " && purchaseInvoice[key].toString().toLowerCase().includes(value.toLowerCase())) {
            isMatch = true
          }
        })
        return isMatch
  ...
  const status = {
    ...
    2: { title: 'To Validate', color: 'light-dark' },
    3: { title: 'Validated', color: 'light-success' }
  }
  ...
  const columns = [
    ...
    // },
    {
      ...
      minWidth: '160px',
      cell: row => {
        return `${row.PIVTYP_0} (${row.INV_TYP})`
      }
    },
    {
      name: 'Invoice Number',
      ...
      cell: row => {
        return (
          <Link to={`/supplier/purchase-invoice/${row.NUM_0}`}>
            {row.NUM_0}
          </Link>
        )
      }
    ...
    },
    {
      ...
      minWidth: '200px',
      cell: row => {
        return (row.PTE_0 && row.PTE_0 !== " ") ? `${row.PTE_0} (${row.TEXTE_0})` : ''
      }
    }
  ]
  ...
  const CustomPagination = () => (
    <ReactPaginate
      ...
      pageCount={(searchValue.length || (fromDate && toDate) || selectedStatus || dueDate) ? filteredData.length / 10 : purchaseInvoices.length / 10 || 1}
      breakLabel={'...'}
  ...
  const onDateChange = (date, type) => {
    if (type === 'from') {
      ...
      if (toDate) {
        ...
        updatedData = purchaseInvoices && purchaseInvoices.filter(item => moment(moment(item.ACCDAT_0).format('YYYY-MM-DD')).isBetween(startDate, endDate, undefined, '[]'))
        setFilteredData(updatedData)
      }
    } else {
      setToDate(date[0])
  ...
  const onDueDateChange = date => {
    setDueDate(date)
    let updatedData = []
    updatedData = purchaseInvoices && purchaseInvoices.filter(item => moment(moment(item.STRDUDDAT_0).format('YYYY-MM-DD')).isSame(moment(date[0]).format('YYYY-MM-DD')))
    setFilteredData(updatedData)
  }
  ...
  const onStatusChange = event => {
    if (event && event.value) {
      ...
      let updatedData = []
      updatedData = purchaseInvoices && purchaseInvoices.filter(item => Number(item.Status) === Number(event.value))
      setFilteredData(updatedData)
    } else {
      setStatus('')
        ...
        <Col sm='12'>
          <Card>
            <CardHeader className='flex-md-row flex-column align-md-items-center border-bottom'>
              ...
              {/* <Button.Ripple color='primary' tag={Link} to='/purchaseInvoices/create'>
            ...
            <DataTable
              noHeader
              pagination
              data={(searchValue.length || (fromDate && toDate) || selectedStatus || dueDate) ? filteredData : purchaseInvoices}
              expandableRows
              columns={columns}
